 ...
 */
 void Calibrate() {
    ...
    Serial.print("mNFEXCalibration Data:");
 ...
 void calibrateBattery() {
          ...
          Serial.print("mNFEXFull battery reading ");
 ...
 */
 void calibrateDAC() {
      //note: added some test points, remove later
      ...
      EEPROM.commit();
      Serial.print("lNFEX");  // flag that voltage range limits coming...
      Serial.print(vlowlimit, 4);
      Serial.print('\t');
      Serial.print(vhighlimit, 4);
      Serial.print('\t');
      Serial.print("cNFEX");  // flag that the linear calibration range has been completed.
 }
...
void calibrateZ() {
    ...
    while (!completedAvailableSettings) {
        ...
        if (!conditionTested && (Voutexpected > 0.09) && (Voutexpected < 1.01)) {  // if we haven't calibrated current settings, and predicted output looks like a suitable range calibrate this setting!
            ...
            Serial.print("cNFEX");  // flag that the linear calibration range has been completed (Android can now Execute_LSR() with compiled data)
            ...
            Serial.print("iNFEX");  // now send Android the latest completed settings code to display (so user knows if more conditions need to be calibrated, with different networkR
        ...
        if (excitationCode > 0x00) {  // so update settings and run through cycle again...
            ...
            Serial.print("yNFEX");  // flag for Android to know that we've calibrated as many settings as give good results with given networkR (so swap out resistor and start calibration again)
        }
    }
}
